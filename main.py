...
Examples:
  ...
  python main.py --input video.mp4 --output result.mp4 --no-display
    ...
    if args.output is None:
        ...
        args.output = str(
            input_path.parent / f"{input_path.stem}_traced{input_path.suffix}"
        )
...
def run_ball_tracking_only(args):
    ...
    while cap.isOpened():
        ...
        if x is not None and y is not None:
            ...
            cv2.putText(
                frame, source, (x + 15, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
            )
        ...
        for i in range(1, len(trail_points)):
            ...
            cv2.line(
                frame, trail_points[i - 1], trail_points[i], (0, 255, 255), thickness
            )
    ...
    print(
        f"Detected in: {detected_frames} frames ({100*detected_frames/total_frames:.1f}%)"
    )
    print(f"\nDetection sources:")
