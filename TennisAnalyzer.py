...
from CourtMapping import courtMap, showLines, showPoint, heightP, widthP, givePoint, yOffset, courtHeight, courtWidth
...
class TennisAnalyzer:
    ...
    def __init__(
        ...
        self.pose_model = YOLO(str(base_dir / 'weights/yolov8_pose.pt'))  # Nano pose model for speed
    ...
    def process_video(
        ...
        if audio_ok:
            ...
            print(f"  Audio shots detected at frames: {audio_shot_frames[:10]}..." if len(audio_shot_frames) > 10 else f"  Audio shots detected at frames: {audio_shot_frames}")
        else:
        ...
        net_y = yOffset + int(courtHeight * 0.5)  # net line in homography/court-map coordinates
        ...
        audio_ball_merge_window_frames = 10  # +/- frames window to match audio peak to net-crossing
        audio_intensity_gate = 0.35  # normalized intensity threshold (0..1)

        def _audio_peak_near(frame_idx: int) -> bool:
            if self.audio_analyzer is None or not self.audio_analyzer.intensity_per_frame:
                return False
        ...
        active_shot = None  # dict with max_speed_kmh updated for a few frames after event
        ...
        while cap.isOpened():
            ...
            if not debug_printed and self.frame_count == 1:
                print(f"First frame - Court detection: source={court_source}, corners={corners}")
                debug_printed = True
            ...
            if court_view is None:
                ...
                cv2.rectangle(court_view, (0, 0), (widthP, heightP), (188, 145, 103), -1)
            ...
            if self.show_players:
                ...
                if player_data:
                    feet1, feet2, hands1, hands2, nose1, nose2, shoulders1, shoulders2, keypoints1, keypoints2 = player_data
                    stats['player_detections'] += 1
                    
                    # Draw pose skeleton overlay
                    if self.show_pose_skeleton:
                        if keypoints1 is not None:
                            annotated_frame = self._draw_skeleton(annotated_frame, keypoints1, self.skeleton_color_p1, 1)
                        if keypoints2 is not None:
                            annotated_frame = self._draw_skeleton(annotated_frame, keypoints2, self.skeleton_color_p2, 2)
                    
                    # Draw player positions
                    if feet1 and feet1[0] and feet1[1]:
                        pos1 = (int((feet1[0][0] + feet1[1][0]) / 2), 
                               int(max(feet1[0][1], feet1[1][1])))
                        player_positions_frame[1] = pos1
                        
                        # Draw on court view
                        if court_view is not None and self.homography_matrix is not None:
                            court_view = showPoint(court_view, self.homography_matrix, pos1)
                            try:
                                p1_map = givePoint(self.homography_matrix, pos1)
                                cv2.circle(court_map_view, (int(p1_map[0]), int(p1_map[1])), 10, self.colors['player1'], -1)
                                player_court_positions[1] = (int(p1_map[0]), int(p1_map[1]))
                            except Exception:
                                pass
                    
                    if feet2 and feet2[0] and feet2[1]:
                        pos2 = (int((feet2[0][0] + feet2[1][0]) / 2), 
                               int(max(feet2[0][1], feet2[1][1])))
                        player_positions_frame[2] = pos2
                        
                        if court_view is not None and self.homography_matrix is not None:
                            court_view = showPoint(court_view, self.homography_matrix, pos2)
                            try:
                                p2_map = givePoint(self.homography_matrix, pos2)
                                cv2.circle(court_map_view, (int(p2_map[0]), int(p2_map[1])), 10, self.colors['player2'], -1)
                                player_court_positions[2] = (int(p2_map[0]), int(p2_map[1]))
                            except Exception:
                                pass
                    ...
                    if hands1 and hands1[1] and nose1:
                        try:
                            radius1 = int(0.65 * euclideanDistance(nose1, pos1 if feet1 else hands1[1]))
                            cv2.circle(annotated_frame, hands1[1], radius1, self.colors['hand_radius'], 1)
                        except:
                            pass
                    if hands2 and hands2[1] and nose2:
                        try:
                            radius2 = int(0.6 * euclideanDistance(nose2, pos2 if feet2 else hands2[1]))
                            cv2.circle(annotated_frame, hands2[1], radius2, self.colors['hand_radius'], 1)
                        except:
                            pass
            ...
            if self.show_court_keypoints and corners:
                annotated_frame = self._draw_court_keypoints(annotated_frame, corners)
            ...
            if player_court_positions:
                ...
                sorted_players = sorted(player_court_positions.items(), key=lambda kv: kv[1][1])
            ...
            if self.show_ball:
                ...
                if x is not None and y is not None:
                    ...
                    cv2.putText(annotated_frame, source[:3].upper(), (x+15, y-5),
                    ...
                    if court_view is not None and self.homography_matrix is not None:
                        try:
                            ...
                            cv2.circle(court_view, (int(ball_court[0]), int(ball_court[1])), 
                                      10, (0, 255, 255), -1)
                            cv2.circle(court_map_view, (int(ball_court[0]), int(ball_court[1])),
                                      10, (0, 255, 255), -1)

                            # Velocity + speed (use previous mapped point)
                            if prev_ball_court_xy is not None and prev_ball_frame is not None:
                                dt = max(1e-6, (self.frame_count - prev_ball_frame) / max(1e-6, fps))
                                ...
                                speed_mps = float(np.sqrt(vx_mps * vx_mps + vy_mps * vy_mps))
                                raw_speed_kmh = float(speed_mps * 3.6)
                                raw_speed_kmh = max(0.0, min(raw_speed_kmh, MAX_BALL_SPEED_KMH))
                                current_ball_speed_kmh = (
                                    (1.0 - SPEED_SMOOTH_ALPHA) * float(current_ball_speed_kmh)
                                    + SPEED_SMOOTH_ALPHA * raw_speed_kmh
                                )
                            prev_ball_court_xy = (int(ball_court[0]), int(ball_court[1]))
                            prev_ball_frame = self.frame_count

                            # Speed samples buffer for per-shot max estimation
                            speed_samples.append((self.frame_count, float(current_ball_speed_kmh)))
                            ...
                            if prev_ball_court_xy is not None:
                                ...
                                cv2.arrowedLine(
                                    court_map_view,
                                    (int(prev_ball_court_xy[0]), int(prev_ball_court_xy[1])),
                                    (ax, ay),
                                    (0, 200, 255),
                                    2,
                                    tipLength=0.25
                            ...
                            if prev_ball_court_y is not None:
                                ...
                                if crossed and moved_enough and (self.frame_count - last_net_cross_frame) > 6:
            ...
            if self.show_trajectory and len(self.ball_trajectory) > 1:
                ...
                for i in range(1, len(trail)):
                    ...
                    cv2.line(annotated_frame, pt1, pt2, self.colors['trajectory'], thickness)
            ...
            map_pip_height = max(1, int(pip_height * 0.55))  # smaller than the homography warp view
            if pip_width > 0 and map_pip_height > 0 and court_map_view is not None:
                ...
                cv2.rectangle(map_pip, (0, 0), (pip_width-1, map_pip_height-1), (100, 100, 100), 1)
                cv2.putText(map_pip, "2D Map", (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
                x_offset = (sidebar_width - pip_width) // 2
                sidebar[sidebar_y:sidebar_y+map_pip_height, x_offset:x_offset+pip_width] = map_pip
            ...
            if pip_width > 0 and pip_height > 0 and court_view is not None:
                ...
                cv2.rectangle(pip, (0, 0), (pip_width-1, pip_height-1), (100, 100, 100), 1)
                cv2.putText(pip, "Homography", (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
                x_offset = (sidebar_width - pip_width) // 2
                y2 = sidebar_y
                y3 = min(height, sidebar_y + pip_height)
                if y3 > y2:
                    sidebar[y2:y3, x_offset:x_offset+pip_width] = pip[:(y3 - y2), :]
                    sidebar_y += (y3 - y2) + 15

            # --- AUDIO SECTION ---
            cv2.putText(sidebar, "AUDIO", (10, sidebar_y + 12), 
            ...
            if self.audio_analyzer is not None:
                ...
                is_audio_shot = self.audio_analyzer.is_shot_frame(self.frame_count, tolerance=2)
                ...
                if self.audio_analyzer.intensity_per_frame:
                    sidebar = draw_audio_waveform(
                        ...
                        self.frame_count,
                        x=10, y=sidebar_y, width=sidebar_width - 20, height=45
                    )
                    sidebar_y += 55
                
                # Draw audio intensity bar in sidebar
                sidebar = draw_audio_overlay(
                    sidebar, intensity, is_audio_shot,
                    x=10, y=sidebar_y, width=sidebar_width - 70, height=18
                )
                sidebar_y += 30
                ...
                is_shot_event = bool(is_ball_net_crossing and _audio_peak_near(self.frame_count))

                if is_shot_event and (self.frame_count - last_shot_frame) > min_shot_gap_frames:
                    ...
                    pre_window = [s for (f, s) in speed_samples if pre_start <= f <= self.frame_count]
                    initial_max = max(pre_window) if pre_window else float(current_ball_speed_kmh)
                    active_shot = {
            ...
            if active_shot is not None:
                active_shot['max_speed_kmh'] = max(float(active_shot['max_speed_kmh']), float(current_ball_speed_kmh))
                active_shot['post_left'] -= 1
                if active_shot['post_left'] <= 0:
                    active_shot['max_speed_kmh'] = max(0.0, min(float(active_shot['max_speed_kmh']), MAX_BALL_SPEED_KMH))
                    shot_events.append(active_shot)
                    hitter_side = active_shot.get('hitter')
                    if hitter_side in player_shot_stats:
                        player_shot_stats[hitter_side]['last_speed_kmh'] = float(active_shot['max_speed_kmh'])
                    active_shot = None
            
            # --- SHOT DETECTED SECTION ---
            cv2.line(sidebar, (10, sidebar_y), (sidebar_width - 10, sidebar_y), (60, 60, 60), 1)
            sidebar_y += 8
            ...
            if frames_since_shot < 30:  # Show for 30 frames (~1 sec)
                ...
                cv2.putText(sidebar, "SHOT DETECTED!", (10, sidebar_y + 22), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, shot_color, 2)
                if last_shot_reason:
                    cv2.putText(sidebar, f"({last_shot_reason})", (10, sidebar_y + 42),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180, 180, 180), 1)
                sidebar_y += 32
                
                # Also flash on main video
                if frames_since_shot < 15:
                    cv2.putText(annotated_frame, "SHOT!", 
                               (width // 2 - 50, 60),
                               cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
            else:
                cv2.putText(sidebar, "Waiting for shot...", (10, sidebar_y + 22), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (80, 80, 80), 1)
                sidebar_y += 32
            
            # Show shot count
            cv2.putText(sidebar, f"Total shots: {shot_count}", (10, sidebar_y + 5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.45, (150, 150, 150), 1)
            sidebar_y += 20
            
            # --- DETECTION INFO ---
            cv2.line(sidebar, (10, sidebar_y + 5), (sidebar_width - 10, sidebar_y + 5), (60, 60, 60), 1)
            sidebar_y += 13
            cv2.putText(sidebar, "DETECTION", (10, sidebar_y + 12), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            sidebar_y += 25
            
            ball_src = source if self.show_ball else 'N/A'
            cv2.putText(sidebar, f"Ball: {ball_src}", (15, sidebar_y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (130, 130, 130), 1)
            sidebar_y += 16
            cv2.putText(sidebar, f"Court: {court_source}", (15, sidebar_y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (130, 130, 130), 1)
            sidebar_y += 16
            cv2.putText(sidebar, f"Frame: {self.frame_count}/{total_frames}", (15, sidebar_y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (130, 130, 130), 1)
            ...
            ry = 10
            cv2.putText(rightbar, "SHOT HISTORY", (10, ry + 14),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.55, (230, 230, 230), 1)
            ry += 26
            cv2.line(rightbar, (10, ry), (right_sidebar_width - 10, ry), (60, 60, 60), 1)
            ry += 12

            cv2.putText(rightbar, f"Cap: {MAX_BALL_SPEED_KMH:.1f} km/h", (10, ry + 12),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.42, (150, 150, 150), 1)
            ry += 18
            cv2.putText(rightbar, f"Top: {player_shot_stats['top']['count']}  Bot: {player_shot_stats['bottom']['count']}", (10, ry + 12),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.42, (150, 150, 150), 1)
            ry += 22
            ...
            for evt in reversed(recent):
                ...
                cv2.rectangle(rightbar, (10, ry), (10 + bar_w, ry + bar_h), (50, 50, 50), -1)
                cv2.rectangle(rightbar, (10, ry), (10 + filled, ry + bar_h), (0, 165, 255), -1)
                cv2.rectangle(rightbar, (10, ry), (10 + bar_w, ry + bar_h), (80, 80, 80), 1)
                ry += bar_h + 12

                cv2.putText(rightbar, f"{speed:5.1f} km/h  {hitter} {lr}", (10, ry),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.42, (230, 230, 230), 1)
                ry += 16
                cv2.putText(rightbar, f"t={tsec:5.1f}s  {reason}", (10, ry),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.38, (140, 140, 140), 1)
                ry += 18
                cv2.line(rightbar, (10, ry), (right_sidebar_width - 10, ry), (50, 50, 50), 1)
                ry += 10
            ...
            cv2.line(final_frame, (sidebar_width, 0), (sidebar_width, height), (80, 80, 80), 2)
            cv2.line(final_frame, (sidebar_width + width, 0), (sidebar_width + width, height), (80, 80, 80), 2)
        ...
        stats['ball_detection_rate'] = 100 * stats['ball_detections'] / self.frame_count if self.frame_count > 0 else 0
        
        return stats
    
    def _create_crop_config(self, x_ratio, x_offset, x_center, y_ratio, y_offset, y_center, width, height):
    ...
    def _detect_court(self, frame, width, height, extraLen):
        ...
        try:
            ...
            hPLines = cv2.HoughLinesP(canny, 1, pi/180, threshold=150, minLineLength=100, maxLineGap=10)
            if hPLines is None:
                return None
            ...
            for i, hPLine1 in enumerate(hPLines):
                ...
                for hPLine2 in hPLines:
                    Line2 = [[hPLine2[0][0], hPLine2[0][1]], [hPLine2[0][2], hPLine2[0][3]]]
                    if Line1 != Line2:
                        x1, y1 = min(Line1[0][0], Line1[1][0]), min(Line1[0][1], Line1[1][1])
                        x2, y2 = max(Line1[0][0], Line1[1][0]), max(Line1[0][1], Line1[1][1])
                        intersect = findIntersection(Line1, Line2, x1-200, y1-200, x2+200, y2+200)
                        if intersect:
                            intersectNum[i][0] += 1
                intersectNum[i][1] = i
            ...
            for i, hPLine in enumerate(hPLines):
                ...
                for p in range(min(8, len(intersectNum))):
                    if i == intersectNum[p][1] and intersectNum[i][0] > 0:
                        cv2.floodFill(nonRectArea, zeros((height+2, width+2), uint8), (x1, y1), 1)
                        cv2.floodFill(nonRectArea, zeros((height+2, width+2), uint8), (x2, y2), 1)
            ...
            # Find extreme lines
            lines = {'xOLeft': None, 'xORight': None, 'yOTop': None, 'yOBottom': None,
                    'xFLeft': None, 'xFRight': None, 'yFTop': None, 'yFBottom': None}
            vals = {'xOLeft': width + extraLen, 'xORight': -extraLen, 
                   'yOTop': height, 'yOBottom': 0,
                   'xFLeft': width + extraLen, 'xFRight': -extraLen,
                   'yFTop': height, 'yFBottom': 0}
            ...
            for hLine in hLines:
                for rho, theta in hLine:
                    ...
                    for axis_name, axis in [('xO', axis_top), ('yO', axis_left), 
                                           ('xF', axis_bottom), ('yF', axis_right)]:
                        intersect = findIntersection(axis, line, -extraLen, 0, width+extraLen, height)
                        if intersect:
            ...
            required = ['xOLeft', 'xORight', 'yOTop', 'yOBottom', 'xFLeft', 'xFRight', 'yFTop', 'yFBottom']
            ...
            # Find corners
            topLeft = findIntersection(lines['xOLeft'], lines['yOTop'], -extraLen, 0, width+extraLen, height)
            topRight = findIntersection(lines['xORight'], lines['yFTop'], -extraLen, 0, width+extraLen, height)
            bottomLeft = findIntersection(lines['xFLeft'], lines['yOBottom'], -extraLen, 0, width+extraLen, height)
            bottomRight = findIntersection(lines['xFRight'], lines['yFBottom'], -extraLen, 0, width+extraLen, height)
    ...
    def _detect_players_yolo_on_crop(self, frame, crop):
        ...
        try:
            ...
            results = self.pose_model(cropped, device=self.pose_device, verbose=False, conf=0.25)
            ...
            if results and len(results) > 0 and results[0].keypoints is not None:
                ...
                if boxes is not None and len(boxes) > 0:
                    ...
                    if best_idx < len(keypoints_data.xy):
                        ...
                        conf = keypoints_data.conf[best_idx].cpu().numpy() if keypoints_data.conf is not None else None
                        ...
                        if conf is not None:
                            for i in range(len(kps_global)):
                                if i < len(conf):
                                    threshold = min_conf_head if i in head_indices else min_conf_body
                                    if conf[i] < threshold:
                        ...
                        if body_keypoints:
                            ...
                            if len(body_keypoints) >= 2:
                                widths = []
                                for idx1, idx2 in [(5, 6), (11, 12)]:  # shoulder pair, hip pair
                                    if idx1 < len(kps_global) and idx2 < len(kps_global):
                                        p1 = kps_global[idx1]
                                        p2 = kps_global[idx2]
                                        if p1[0] > 0 and p2[0] > 0:
                                            widths.append(abs(p1[0] - p2[0]))
                                if widths:
                                    body_size = max(widths) * 4  # Allow 4x body width as max distance
                            
                            # Filter keypoints too far from body center
                            for i in range(len(kps_global)):
                                x, y = kps_global[i]
                                if x > 0 and y > 0:
                                    dist = np.sqrt((x - body_center_x)**2 + (y - body_center_y)**2)
                                    if dist > body_size:
                                        kps_global[i] = [0, 0]  # Outlier - too far from body
    ...
    def _draw_court_keypoints(self, frame, corners, keypoints_flat=None):
        ...
        if corners:
            ...
            for i, corner in enumerate(corners):
                if corner:
                    ...
                    # Draw label
                    cv2.putText(frame, labels[i], (corner[0]+12, corner[1]+5),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.court_kp_color, 2)
    ...
    def _detect_players(self, frame, crop1, crop2):
        ...
        try:
            ...
            if feet1 or feet2 or hands1 or hands2:
                return (feet1, feet2, hands1, hands2, nose1, nose2, shoulders1, shoulders2, keypoints1, keypoints2)
            return None
    ...
    def _draw_info_overlay(self, frame, ball_source, court_source='N/A'):
        ...
        # Frame info
        cv2.putText(frame, f"Frame: {self.frame_count}", (20, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(frame, f"Ball: {ball_source}", (20, 50),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        cv2.putText(frame, f"Court: {court_source}", (20, 70),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 1)
        cv2.putText(frame, f"Trajectory: {len(self.ball_trajectory)} pts", (20, 90),
...
def run_full_analysis(args):
    ...
    for source, count in sorted(stats['detection_sources'].items(), key=lambda x: -x[1]):
        pct = 100 * count / stats['processed_frames']
        print(f"  {source:12s}: {count:4d} ({pct:.1f}%)")
    
    if 'court_sources' in stats:
        print(f"\nCourt Detection Sources:")
        for source, count in sorted(stats['court_sources'].items(), key=lambda x: -x[1]):
            pct = 100 * count / stats['processed_frames']
            print(f"  {source:12s}: {count:4d} ({pct:.1f}%)")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Tennis Analyzer - Full video analysis')
    parser.add_argument('--input', '-i', required=True, help='Input video path')
    parser.add_argument('--output', '-o', default='output/analyzed.mp4', help='Output video path')
