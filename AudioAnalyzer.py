...
class AudioAnalyzer:
    ...
    def extract_audio(self):
        ...
        try:
            ...
            if os.path.exists(temp_wav.name) and os.path.getsize(temp_wav.name) > 0:
                ...
                print(
                    f"  Audio extracted: {len(self.audio_data)} samples at {self.sample_rate}Hz"
                )
                return True
            else:
    ...
    def analyze_intensity(self):
        ...
        if self.intensity_per_frame:
            ...
            if max_intensity > 0:
                self.intensity_per_frame = [
                    i / max_intensity for i in self.intensity_per_frame
                ]
    ...
    def get_intensity_at_frame(self, frame_idx):
        ...
        if self.intensity_per_frame is None or frame_idx >= len(
            self.intensity_per_frame
        ):
            return 0.0
...
def draw_audio_overlay(
    frame, intensity, is_shot=False, x=20, y=None, width=200, height=30
):
    ...
    if fill_width > 0:
        ...
        cv2.rectangle(
            frame, (x + 2, y + 2), (x + fill_width - 2, y + height - 2), color, -1
        )
...
def draw_audio_waveform(
    frame, intensities, current_frame, x=20, y=None, width=300, height=50
):
    ...
    cv2.line(
        frame, (x, threshold_y), (x + width, threshold_y), (0, 0, 150), 1, cv2.LINE_AA
    )
