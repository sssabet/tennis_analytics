...
class CourtSideDetector:
    ...
    def __init__(
        self,
        model_path: str = "weights/courtside_yolo.pt",
...
class BallTrackerFusion:
    ...
    def detect_ball(
        ...
        # Fuse detections
        final_pos, source = self._fuse_detections(
            tracknet_pos, courtside_pos, courtside_conf
        )
        ...
        if not self.kalman.initialized:
            ...
            if final_pos is not None:
                ...
                gate = min(
                    self.kalman_gate_max_px,
                    self.kalman_gate_base_px + self.kalman_gate_vel_mult * vel_mag,
                )
                dist_meas = float(
                    np.sqrt((final_pos[0] - kx) ** 2 + (final_pos[1] - ky) ** 2)
                )
        ...
        return (
            final_pos[0] if final_pos else None,
            final_pos[1] if final_pos else None,
            source,
        )
            ...
            if dist <= self.max_detection_distance:
                ...
                fused_x = int(
                    (
                        ...
                        + courtside_pos[0] * adjusted_courtside_weight
                    )
                    / total_weight
                )
                fused_y = int(
                    (
                        ...
                        + courtside_pos[1] * adjusted_courtside_weight
                    )
                    / total_weight
                )
                ...
                if self.kalman.initialized and self.kalman.is_tracking_valid():
                    kx, ky = self.kalman.get_position()
                    dist_tracknet = np.sqrt(
                        (tracknet_pos[0] - kx) ** 2 + (tracknet_pos[1] - ky) ** 2
                    )
                    dist_courtside = np.sqrt(
                        (courtside_pos[0] - kx) ** 2 + (courtside_pos[1] - ky) ** 2
                    )
